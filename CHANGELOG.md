...
- Карточка по номеру свидетельства разбирает живую вёрстку открытого реестра ФИПС: правообладатель, классы МКТУ, срок (в том числе продление), статус.
- Если документ есть, а содержательные поля разобрать нельзя — ответ `parse_failed` со ссылкой на первоисточник, не пустое «найдено».
